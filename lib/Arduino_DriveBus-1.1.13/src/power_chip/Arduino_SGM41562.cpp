/*
 * @Description: Arduino_SGM41562.cpp
 * @version: V1.0.0
 * @Author: Xk_w
 * @Date: 2023-11-16 15:42:22
 * @LastEditors: Please set LastEditors
 * @LastEditTime: 2024-09-13 22:56:58
 * @License: GPL 3.0
 */
#include "Arduino_SGM41562.h"

Arduino_SGM41562::Arduino_SGM41562(
...
bool Arduino_SGM41562::begin(int32_t speed)
{
    return Arduino_IIC::begin(speed);
}

bool Arduino_SGM41562::IIC_Initialization(void)
{
    ...
    if (_bus->BufferOperation(_device_address, SGM41562_Initialization_BufferOperations,
                              sizeof(SGM41562_Initialization_BufferOperations)) == false)
    {
        return false;
    }
...
int32_t Arduino_SGM41562::IIC_Device_ID(void)
{
    uint8_t temp_buf = 0;
    if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_DEVICE_ID, &temp_buf) == true)
    {
...
bool Arduino_SGM41562::IIC_Write_Device_State(uint32_t device, uint8_t state)
{
    uint8_t temp_buf = 0;
    ...
    case Arduino_IIC_Power::Device::POWER_DEVICE_CHARGING_MODE:
        ...
        case Arduino_IIC_Power::Device_State::POWER_DEVICE_ON:
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_POWER_ON_CONFIGURATION, &temp_buf) == true)
            {
                temp_buf &= 0B11110111;

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_POWER_ON_CONFIGURATION, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device::POWER_DEVICE_WATCHDOG_MODE:
        ...
        {
        case Arduino_IIC_Power::Device_State::POWER_DEVICE_ON:
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_CHARGE_TERMINATION_TIMER_CONTROL, &temp_buf) == true)
            {
                temp_buf |= 0B10000000;

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_CHARGE_TERMINATION_TIMER_CONTROL, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device::POWER_DEVICE_NTC_MODE:
        ...
        {
        case Arduino_IIC_Power::Device_State::POWER_DEVICE_ON:
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_MISCELLANEOUS_OPERATION_CONTROL, &temp_buf) == true)
            {
                temp_buf |= 0B10000000;

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_MISCELLANEOUS_OPERATION_CONTROL, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device::POWER_DEVICE_PCB_OTP_MODE:
        ...
        {
        case Arduino_IIC_Power::Device_State::POWER_DEVICE_ON:
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_SYSTEM_VOLTAGE_REGULATION, &temp_buf) == true)
            {
                temp_buf |= 0B10000000;

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_SYSTEM_VOLTAGE_REGULATION, temp_buf) == true)
                {
                    return true;
                }
            }
...
bool Arduino_SGM41562::IIC_Write_Device_Value(uint32_t device, int64_t value)
{
    uint8_t temp_buf = 0;
    ...
    case Arduino_IIC_Power::Device_Value::POWER_DEVICE_MINIMUM_INPUT_VOLTAGE_LIMIT: // 3880mV-5080mV(步进值：80mV)
        ...
        {
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_INPUT_SOURCE_CONTROL, &temp_buf) == true)
            {
                temp_buf &= 0B00001111; // 清空
                temp_buf = temp_buf | (((value - 3880) / 80) << 4);

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_INPUT_SOURCE_CONTROL, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device_Value::POWER_DEVICE_CHARGING_TARGET_VOLTAGE_LIMIT: // 3600mV-4545mV(步进值：15mV)
        ...
        {
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_CHARGE_VOLTAGE_CONTROL, &temp_buf) == true)
            {
                temp_buf &= 0B00000011; // 清空
                temp_buf = temp_buf | (((value - 3600) / 15) << 2);

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_CHARGE_VOLTAGE_CONTROL, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device_Value::POWER_DEVICE_FAST_CHARGING_CURRENT_LIMIT: // 8mA-456mA(步进值：8mA)
        ...
        {
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_CHARGE_CURRENT_CONTROL, &temp_buf) == true)
            {
                temp_buf &= 0B11000000; // 清空
                temp_buf = temp_buf | ((value - 8) / 8);

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_CHARGE_CURRENT_CONTROL, temp_buf) == true)
                {
                    return true;
                }
            }
    ...
    case Arduino_IIC_Power::Device_Value::POWER_DEVICE_TERMINATION_PRECHARGE_CHARGING_CURRENT_LIMIT: // 1mA-31mA(步进值：2mA)
        if (value >= 1 && value <= 31)
        {
            if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_WR_DISCHARGE_TERMINATION_CURRENT, &temp_buf) == true)
            {
                temp_buf &= 0B11110000; // 清空
                temp_buf = temp_buf | ((value - 1) / 2);

                if (_bus->IIC_WriteC8D8(_device_address, SGM41562_RD_WR_DISCHARGE_TERMINATION_CURRENT, temp_buf) == true)
                {
                    return true;
                }
            }
...
String Arduino_SGM41562::IIC_Read_Device_State(uint32_t information)
{
    uint8_t temp_buf = 0;
    ...
    case Arduino_IIC_Power::Status_Information::POWER_CHARGING_STATUS:
        if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_SYSTEM_STATUS, &temp_buf) == true)
        {
            temp_buf &= 0B00011000;
        ...
        return "->Read SGM41562_RD_SYSTEM_STATUS fail";
        break;
    ...
    case Arduino_IIC_Power::Status_Information::POWER_INPUT_FAULT_STATUS:
        if (_bus->IIC_ReadC8D8(_device_address, SGM41562_RD_FAULT, &temp_buf) == true)
        {
            temp_buf &= 0B00100000;
        ...
        return "->Read SGM41562_RD_FAULT fail";
        break;
...
double Arduino_SGM41562::IIC_Read_Device_Value(uint32_t information)
{
    uint8_t temp_buf = 0;
