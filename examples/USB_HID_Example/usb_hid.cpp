...
static  struct HIDMessage pdat = {0};
static int _maxX, _maxY;
...
static void hid_host_mouse_report_callback(const uint8_t *const data, const int length)
{
    ...
    if (queue_in) {
        pdat.id = 'm';
        pdat.left = mouse_report->buttons.button1;
        pdat.right = mouse_report->buttons.button2;
        x_pos = constrain(x_pos, 0, _maxX);
        y_pos = constrain(y_pos, 0, _maxY);
        pdat.x = x_pos;
        pdat.y = y_pos;
        // Serial.printf("X: %06d\tY: %06d\t|%c|%c|\r",
        //               x_pos, y_pos,
        //               (mouse_report->buttons.button1 ? 'o' : ' '),
        //               (mouse_report->buttons.button2 ? 'o' : ' '));
        // Serial.println();

        xQueueSend(queue_in, &pdat, portMAX_DELAY);
    }
...
static void usb_lib_task(void *arg)
{
    ...
    while (gpio_get_level(APP_QUIT_PIN) != 0) {
        ...
        if (event_flags & USB_HOST_LIB_EVENT_FLAGS_NO_CLIENTS) {
            ...
            Serial.println("USB Event flags: NO_CLIENTS");
        }
        // All devices were removed
        if (event_flags & USB_HOST_LIB_EVENT_FLAGS_ALL_FREE) {
            Serial.println("USB Event flags: ALL_FREE");
        }
    }
    // App Button was pressed, trigger the flag
    user_shutdown = true;
    Serial.println("USB shutdown");
    // Clean up USB Host
...
void setupUSBHID(QueueHandle_t queue_i, int x_max, int y_max)
{
    ...
    Serial.println("HID Host example");
